...
} from './react-scrollable.type';
import { useEffect, useMemo, useRef } from 'react';
import { OverlayScrollbars } from 'overlayscrollbars';

export const createDefer = (): ROSDefer => {
  /* c8 ignore start */
...
export const useOverlayScrollbars = (
  ...
  const [requestDefer, cancelDefer] = useMemo<ROSDefer>(createDefer, []);
  ...
  return useMemo<[UseOverlayScrollbarsInitialization, UseOverlayScrollbarsInstance]>(
    () => [
      (target) => {
        ...
        const init = () => (instanceRef.current = OverlayScrollbars(target, currOptions || {}, currEvents || {}));
